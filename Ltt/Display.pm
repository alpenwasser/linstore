...
sub _concat_fields
{
	...
	for my $system_id (keys %{ $systems_ref })
	{
		...
		for (@{ $fields_ref })
		{
            # Make  sure  we  don't   add  a  comma  at  the
            # beginning of string.
			if ($concat_string)
			{
				$concat_string .= ", " . $systems_ref->{$system_id}{$_};
			}
			else
			{
				$concat_string = $systems_ref->{$system_id}{$_};
		...
		$systems_ref->{$system_id}{$new_field_name} = $concat_string;
	}
}


sub _format_capacities
{
	my $systems_ref		= shift;
...
sub _prepare_ranks
{
	...
	my $rank_length = _get_max_rank_length($systems_ref);

	# Ranks get padded on left for the units to line up.
	_pad_ranks($systems_ref,$rank_length);
...
sub _prepare_usernames
{
    ...
    # padding to its right in separate strings because we do
    # not want  to make  the padding  part of  the hyperlink
    # from [post=...]username[/post].

	my $username_length = _get_max_field_length($systems_ref,"username");
...
sub _prepare_capacities
{
    ...
    # trailing ".0"  if they are integers,  then padded from
    # the left for the units to line up properly.

	_format_capacities($systems_ref);
	my $capacity_length 
		= _get_max_field_length($systems_ref,"formatted_capacity");
	_pad_field_left(
		$systems_ref,
		"formatted_capacity",
		$capacity_length,
...
sub _prepare_enclosures
{
	...
	my $enclosure_length = _get_max_field_length($systems_ref,"case");
...
sub _prepare_os_storage_sys
{
    ...
    # Combine the OS  and storage system fields  into one to
    # keep  the  line  length short  enough  for  acceptable
    # formatting on smaller screens.
	...
	my $os_storage_length
		= _get_max_field_length($systems_ref,"os_storage_sys");
	...
	say for map
			{
				my $row 
					...
					= $systems_ref->{$_}{padded_padded_rank}
                ...
                # defined...
				$row .= 
					(" " x $constants_ref->{before_notes_padding})
					. $systems_ref->{$_}{notes} 
			...
			sort
			{
                # The ranks have  already been determined at
                # this point  by the Ranking module  and are
                # stored in the {rank} field.
...
sub _print_total_capacity
{
    ...
    # At this point,  total_combined_capacity should not yet
    # have  been  calculated, but  if  it  has been  and  is
    # already stored, don't recalculate.
...
sub generate_list_footer
{
	...
	_print_total_capacity($systems_ref, $constants_ref, $hdd_types_ref);
...
sub _prepare_system_summary_stats
{
    ...
    # Make sure keys  for %{ $titles_ref }  are identical to
    # the corresponding keys in %{ $constants_ref }.
	my $titles_ref = 
	{
	...
	$constants_ref->{$_} = $titles_ref->{$_} for keys %{ $titles_ref };


	return (
		format_number(get_arithmetic_mean($capacities_ref)),	# mean
		format_number(get_median($capacities_ref)),				# median
		get_mode($capacities_ref),								# mode, 
																# unique values
	);
}
...
sub _prepare_hdd_summary_stats
{
	...
	my $titles_ref = 
	{
		"total_drives"		=> $constants_ref->{total_drives},
		"avg_dr_per_system"	=> $constants_ref->{avg_dr_per_system},
		"total_comb_cap"	=> $constants_ref->{total_comb_cap},
		"avg_dr_cap"		=> $constants_ref->{avg_dr_cap},
		"med_dr_cap"		=> $constants_ref->{med_dr_cap}
	};
	...
	my $avg_drives_per_system = get_avg_drives($systems_ref,$total_drives);

    # The  total  combined  capacity should  at  this  point
    # already   have   been   calculated   and   stored   in
    # {total_combined_capacity}, but if  for whatever reason
    # that is not the case, calculate and store it now.
	$constants_ref->{total_combined_capacity} = (
		($constants_ref->{total_combined_capacity}) 
		? 
		$constants_ref->{total_combined_capacity}
	...
	my $med_drive_cap = get_med_drive_cap($hdd_configs_ref, $hdd_types_ref);
...
sub _prepare_hdd_size_stats
{
	...
	my $counts_ref	= get_hdd_counts_by_size($hdd_configs_ref,$hdd_types_ref);
	...
	return (
		...
		get_hdd_perc_cap_by_size(	$sums_ref,
									$constants_ref->{total_combined_capacity})
	);
}
...
sub _prepare_hdd_vendor_stats
{
	...
	my ($counts_ref, $sums_ref)	
		= get_hdd_counts_and_caps_by_vendor($hdd_configs_ref,$hdd_types_ref);

	return (
		$counts_ref,
		$sums_ref,
		get_hdd_perc_count_by_vendor($counts_ref),
		get_hdd_perc_cap_by_vendor(	$sums_ref,
									$constants_ref->{total_combined_capacity})
	);
}
...
sub _print_hdd_summary_stats
{
    ...
    # Strip superfluous dot and tenths. Not using int() here
    # because  that would  also remove  the padding  we just
    # added.
...
sub _format_columns
{
    # Get maximum  length of values  for a hash  (passed via
    # reference), then pad remaining  values of hash to same
	...
	{
		my $max_length
			= get_max_elem_length( [ values %{ $hash_ref } ] );
...
sub _print_hdd_size_stats
{
	...
	_format_columns($constants_ref->{hdd_table_col_spacing_size},@_);
    ...
    # Calculate Length  difference for first  column between
    # HDD  stats  by size  table  and  HDD stats  by  vendor
    # table. Use result to pad first column of by size table
    # to  align its  following  columns with  the by  vendor
    # table.
	my $length_difference = abs
	(
		get_max_elem_length( [ keys %{ $hdd_counts_by_size_ref } ] ) 
		+ length($constants_ref->{capacity_unit}) 
		- get_max_elem_length( $constants_ref->{vendor_list})
	);
...
sub _print_hdd_vendor_stats
{
	...
	_format_columns($constants_ref->{hdd_table_col_spacing_vendor},@_);
...
sub generate_statistics
{
    ...
    # formatting purposes.
	_print_hdd_summary_stats($constants_ref,
							$total_drive_count, 
							$avg_drives_per_system,
							$constants_ref->{total_combined_capacity} 
								. $constants_ref->{capacity_unit},
	...
	_print_hdd_size_stats(	$constants_ref,
